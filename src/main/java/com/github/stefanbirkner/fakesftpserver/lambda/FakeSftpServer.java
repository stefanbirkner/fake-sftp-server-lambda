...
public class FakeSftpServer {
    ...
    /**
     * Register a user for public key authentication. {@code FakeSftpServer}
     * generates a key pair for the user and returns the pair's private key.
     * The private key has to be used for authentication.
     * <p>If {@code addSshKeyForUser} is called multiple times with the same
     * username then the latest key is effective.
     * <p>After registering a user it is only possible to connect to the server
     * with the SSH key or one of the registered username/password pairs.
